---
name: commit-message
description: Create a brief commit message from staged git diff.
---

# Commit Message

Create a brief commit message that reflects only the essence of the staged git diff.
...
Do not announce, explain, narrate tool use, or print command commentary.
...
If nothing is staged, return only:
...
- Return only the commit message or the exact fallback message.
- The title must be one line and reflect the whole diff.
- Use bullets only for significant changes, not every changed file.
